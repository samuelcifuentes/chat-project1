...
function buildLayout() {
    ...
    dom.sendAudioFileBtn = document.getElementById('sendAudioFileBtn');
...
function bindEvents() {
    ...
    dom.audioFileInput.addEventListener('change', handleAudioFileSelected);
    dom.sendAudioFileBtn.addEventListener('click', sendSelectedAudioFile);
...
async function registerUser() {
    ...
    try {
        ...
        handleError('No se pudo registrar', error);
    }
}
...
async function createGroup() {
    ...
    try {
        ...
        handleError('No se pudo crear el grupo', error);
    }
}
...
async function loadHistory() {
    ...
    try {
        ...
        handleError('Error al recuperar historial', error);
    }
}
...
async function sendTextMessage() {
    ...
    try {
        ...
        handleError('No se pudo enviar', error);
    }
}
...
async function uploadAudio() {
    ...
    try {
        ...
        pushNotification('Nota de voz enviada.', 'info');
    } catch (error) {
        handleError('Error enviando audio', error);
    }
}

function handleAudioFileSelected(event) {
    const [file] = event.target.files;
    state.pendingAudioFile = file || null;
    dom.sendAudioFileBtn.disabled = !state.pendingAudioFile;
    if (file) {
        pushNotification(`Nota seleccionada: ${file.name}`, 'info');
    }
}
...
async function sendSelectedAudioFile() {
    ...
    if (!state.pendingAudioFile) {
        pushNotification('Selecciona un archivo de audio.', 'error');
        return;
    }
    try {
        const buffer = await state.pendingAudioFile.arrayBuffer();
        const { id, type } = getTarget();
        await state.chatPrx.sendAudio(
            state.user.id,
            id,
            type,
            new Uint8Array(buffer),
            state.pendingAudioFile.type || 'audio/webm'
        );
        pushNotification('Nota de voz enviada.', 'info');
...
function clearSelectedAudioFile() {
    ...
    dom.sendAudioFileBtn.disabled = true;
}
...
async function triggerCall(action) {
    ...
    try {
        ...
        handleError('Error con la llamada', error);
    }
}
...
function handleError(prefix, error) {
    ...
    pushNotification(`${prefix}: ${details}`, 'error');
}
